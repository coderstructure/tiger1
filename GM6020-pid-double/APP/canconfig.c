...
#include "stdio.h"
#include "canconfig.h"
int16_t PreEncoder,RxEncoder;
int Round;
int16_t TotalEncoder;
//标准CAN ID 
...
{   
  ...
  uint8_t data[8] = {0};  
	//结构体接收数据
   motor_info_t motor_yaw_info;
	 motor_yaw_info.rotor_angle =(data[0] <<8  | data[1])/8192.0f *360.0f;
	 motor_yaw_info.rotor_speed=(int16_t)((data[2] <<8    | data[3]));
	 motor_yaw_info.torque_current=(int16_t)((data[4] <<8 | data[5]));
...
//	bsp_can1_filter_config();
  ...
  if(HAL_CAN_GetRxFifoFillLevel(&hcan1,CAN_RX_FIFO0) != 0)
  {
    ...
    int16_t Delta_Ecounter ;
	  PreEncoder = RxEncoder;
	  RxEncoder    = ((rx_data[0] << 8) | rx_data[1]);
    ...
    motor_yaw_info.temp           =   rx_data[6];
	  ...
	  Delta_Ecounter = RxEncoder -PreEncoder ;
	  if(Delta_Ecounter<-4096)
		{
		  Round++;
		}
		else if(Delta_Ecounter>4096)
		{
		   Round--;
		}
		TotalEncoder =Round*8192.0f +RxEncoder;
